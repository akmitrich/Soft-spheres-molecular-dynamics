...
fn main() {
    ...
    job.run(5);
    println!("Run 1 complete. vel_sum = {:?}. Must be zero.", job.vel_sum());
    job.run(5);
    println!("Run 2 complete. vel_sum = {:?}. Must be zero.", job.vel_sum());
}
...
fn create_job() -> Job<3> {
    ...
    match Track::restore_from("track.txt") {
        Ok(restored_state) => builder
            .state(restored_state),
        Err(default_state) => builder
            .state(default_state)
            .init_pos(pos)
            .random_vel(1.),
    }.job()
